...
async function existingToken(context: Context) {
  ...
  if (!id || id > 2147483647) return { error: apiError(400, 'invalid_request', 'Invalid token id.') } as const;
...
export async function PATCH(request: Request, context: Context) {
  return handleApiRequest(request, 'system:super_admin', async (_principal, audit) => {
    ...
    if (queryError) return apiError(400, 'invalid_request', queryError);
    const id = parsePositiveId((await context.params).id);
    if (!id || id > 2147483647) return apiError(400, 'invalid_request', 'Invalid token id.');
