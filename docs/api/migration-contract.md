...
Both use shared `getCalendarPage`/`getCalendarItems` services and live authentication. Entries retain `id`, `kind`, `name`, `description`, UTC `eventDate`, nullable UTC `startsAtUtc`, UTC date-only `dateKey`, and `href`, plus operation `isSideOp` or session `status`/nullable `trainerName`. Operation event dates fall back from start time to event date to creation date. `startsAtUtc` preserves the scheduled instant separately: null for date-only operations, and the session start time for training sessions. Staff links point to administration; other users’ links derive only from their own active attendee request. No attendee records are returned.
...
The route delegates GET to the shared public `getPublicOrbatList` handler in `lib/api/orbat-list.ts`. POST creation is migrated separately in the next batch. Tests import the actual route; coverage includes the delegated handlers through `lib/api/**`. The collection route is included in the coverage gate once both delegates are migrated, and its public GET/protected POST status is reported separately in the inventory. Anonymous event-stream regression coverage remains future work.

## Eighteenth migration batch: ORBAT creation

`POST /api/orbats` requires `orbat:create` for a live session user or an active superadmin bot token and returns `201` with `data: { id }`. Public GET on the same URL is unchanged. Creation now uses the shared protected handler, structured errors, strict nested payload validation, and transactional audit/outbox behavior.

Creation requires a trimmed nonempty operation name and a nonempty squad array. Each squad has a nonempty name, nonnegative Int32 order index, and nonempty slot array. Slots accept only optional nullable positive Int32 `squadRoleId`, nonnegative Int32 `orderIndex`, and positive Int32 `maxSignups`. Squad positions and slot positions within a squad must be unique. Client IDs, slot names, deleted markers, and per-slot frequency aliases are rejected. Operation text fields are nullable and trimmed; `isSideOp` is boolean. Linked frequency IDs must be unique positive Int32 values, and temporary frequencies require all five fields: nonempty `frequency`, `type` (`SR`/`LR`), boolean `isAdditional`, and string `channel`/`callsign` (which may be empty).

Date inputs are nullable `eventDateUtc`, `startsAtUtc`, and `endsAtUtc`, requiring explicit timezones and normalization to UTC. Legacy `eventDate`, `startTime`, and `endTime` create payload aliases are rejected. End requires start and must be later; start cannot be in the past, and date-only scheduling via `eventDateUtc` cannot target a past UTC day. The website emits only the canonical create fields. Timed operations use the browser local timezone converted to UTC; a selected date without a start time means that UTC calendar day and is sent as an explicit midnight-UTC `eventDateUtc` timestamp. Plain `YYYY-MM-DD` timestamp inputs are not accepted. Creation strips temporary-frequency UI `_id` values and slot display names. The legacy edit payload keys remain; date-only form initialization and submission now also preserve the UTC calendar date, avoiding timezone-dependent date shifts on a later edit.

Missing role/frequency references return 404, retired roles return 409, invalid payloads 422, and malformed JSON or any POST query parameters return 400. Operation, squads, slots, frequency links, metadata-only `orbat.created` audit, and the existing outbox event commit in a serializable transaction. Audits exclude briefing text; realtime notifications are emitted safely after commit. Session creators retain their user ID; bot-created operations have a null creator, with the bot-token actor captured in the audit.

Deploy the generated `20260918030000_optional_orbat_creator` Prisma migration before using bot creation. It makes the creator relation optional while preserving Restrict behavior for existing user references. The migration was generated from schema differences without applying it to the development database. Both collection methods and their shared helpers are now included in the API coverage gate.
...
The enforced requirement is 100% lines, branches, functions, and statements across API handlers and supporting services. Publish CI reports and gate merges on tests and coverage. A threshold applied only to migrated modules is an interim batch guard, not repository-wide completion. Endpoint tests and actual coverage reports must establish readiness; inventory test references alone do not.
...
`npm run test:backend` combines unit and isolated Prisma integration projects with one coverage map covering every API route and every library (only declaration files excluded). The enforced minimum is 100% statements, branches, functions and lines; the quick unit-only report is no longer the release gate. See [testing](./testing.md) for exact commands and emulator limits.
