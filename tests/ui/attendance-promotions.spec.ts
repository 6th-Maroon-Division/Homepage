...
test('automatic promotion button applies a real promotion and reports API failures', async ({ page, login, db }) => {
  ...
  expect((await db.userRank.findUniqueOrThrow({ where: { userId: user.id } })).currentRankId).toBe(high.id);
  expect(await db.rankHistory.count({ where: { userId: user.id } })).toBe(1);
  await page.route('**/api/ranks/promotions/automatic', route => route.fulfill({ status: 403, contentType: 'application/json', body: JSON.stringify({ error: { message: 'Requires permission: rank:manage_promotions' } }) }));
  await page.getByRole('button', { name: 'Auto Rankup', exact: true }).click();
  await expect(page.locator('p[role="alert"]')).toContainText('Requires permission: rank:manage_promotions');
  await page.route('**/api/ranks/promotions/automatic', route => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ data: { promotedCount: 0, errorsCount: 1, ineligibleCount: 0 }, meta: {} }) }));
  await page.getByRole('button', { name: 'Auto Rankup', exact: true }).click();
  await expect(page.locator('p[role="alert"]')).toContainText('finished with errors');
});
