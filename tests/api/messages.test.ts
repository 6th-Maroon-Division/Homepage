...
test.each([null,{},[],{...payload,message:'alias'},{...payload,title:''},{...payload,body:'x'.repeat(10001)},{...payload,type:'bad'},{...payload,actionUrl:'javascript:alert(1)'},{...payload,audience:'all'},{...payload,audience:{type:['all']}},{...payload,audience:{type:'users',userIds:['4']}},{...payload,audience:{type:'users',userIds:[4,4]}},{...payload,audience:{type:'all',userIds:[]}},{...payload,audience:{type:'rank'}}])('strict send validation %j',async body=>{expect((await POST(req('POST',body))).status).toBe(422);expect(m.db.message.create).not.toHaveBeenCalled()});
