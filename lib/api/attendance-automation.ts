...
export async function compileAttendance(request: Request, principal: ApiPrincipal, audit: ApiAuditContext, orbatId: number) {
  ...
  const result = await prisma.$transaction(async tx => {
    const orbat = await tx.orbat.findUnique({ where: { id: orbatId } }); if (!orbat) fail(404, 'Operation not found.'); if (orbat.isSideOp) fail(409, 'Attendance compilation is disabled for side operations.');
    const { startsAtUtc: start, endsAtUtc: end } = resolveOrbatScheduleWindow(orbat); if (!start || !end) fail(422, 'Operation start and end are required for compilation.');
    const signups = await tx.signup.findMany({ where: { slot: { orbatId } }, select: { id: true, userId: true }, orderBy: { id: 'asc' } });
    for (const signup of signups) await requireAttendanceUser(tx, principal, signup.userId, 'attendance:edit');
    ...
    for (const signup of signups) {
      ...
      await tx.attendanceLog.create({ data: { attendanceId: saved.id, action: 'compiled', source: 'automation', changedById: principal.kind === 'user' ? principal.userId : null } });
